...
function handleFile(e) {
  ...
  reader.onload = function (e) {
    ...
    sheetList.forEach((sheetName) => {
        ...
        <a class='link-a' id='element-${number}' href='#' onclick='mostrarTabla(${JSON.stringify(sheetName)}, ${number})'> 
        ${sheetName}
        </a></li>`;
      number++;
    });
...
function mostrarTabla(sheet, condition) {
    if (condition===0){
        console.log("lo tiene ", sheet)
    }else{
        console.log("no lo tiene ", sheet, '-', condition)
        //console.log(excel.AGE_Actualizar_Datos)
        sheet = excel[sheet];
